...
5. If the same binary is configured globally and individually for backups, individual settings take precedence. If different binaries are configured globally and individually, the Operator combines the settings when running a backup.
6. For a restore, individual settings always take precedence over global ones.
