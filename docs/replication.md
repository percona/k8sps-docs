...
* **No automatic rejoin to ClusterSet after replication stops** — The Operator does not auto-rejoin a replica after pause, stop, or a stopped replication channel. You must restore replication manually. Determine the safe window and trigger rejoin. See [Rejoin a replica cluster](replication-setup.md#rejoin-a-replica-cluster).
