...
* **Errors in the list**: The Operator processes `spec.users` in order and stops at the first entry that fails to reconcile (for example, invalid grants or a missing password Secret). Entries listed after the failing one are left unprocessed during that reconcile. The Operator retries automatically on the next reconcile loop, so once you fix the failing entry, processing resumes and continues down the list until all users are synced.
