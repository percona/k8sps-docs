...
* [K8SPS-456](https://perconadev.atlassian.net/browse/K8SPS-456),[K8SPS-550](https://perconadev.atlassian.net/browse/K8SPS-550) - Fixed the issue with service accounts defined for HAProxy Pods via Custom Resource not being applied.

* [K8SPS-556](https://perconadev.atlassian.net/browse/K8SPS-556) - Improved logging when telemetry server is unavailable by placing them to Debug level
...
* [K8SPS-598](https://perconadev.atlassian.net/browse/K8SPS-598) - Fixed the issue with unneeded cluster restart after the Operator image update. The Operator now prevents these restarts by adding the logic to compare the Operator's new image version with the Custom Resource version, ensuring the init container image in the StatefulSet is only updated when necessary.
