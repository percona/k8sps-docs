# Install Percona Server for MySQL on Rancher Kubernetes Engine (RKE2)

This guide shows you how to deploy Percona Operator for MySQL on
...
1. **Linux hosts** that meet the [RKE2 requirements :octicons-link-external-16:](https://docs.rke2.io/install/requirements). For a production-like setup, use at least 3 nodes so the Operator can schedule a MySQL cluster according to the [system requirements](System-Requirements.md#resource-limits).
...
3. The creation process may take some time. When the process is over your
    ...
    ??? example "Expected output"
        ...
        ps-cluster1   group-replication   ps-cluster1-haproxy.default   ready   3                      3                  5m26s
        ```

You have deployed Percona Server for MySQL with the default configuration, which includes three HAProxy and three Percona Server for MySQL instances.

To install Percona Server for MySQL with customized parameters, see [Install Percona Operator for MySQL with customized parameters](custom-install.md).
